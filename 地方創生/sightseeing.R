...
correlation <- cor.test(sightseeing$平均回数, sightseeing$観光地点数)
correlation
...
correlation <- cor.test(sightseeing$平均回数, sightseeing$自然)
correlation
abline(model, col = "red")
summary(model)
...
abline(model, col = "red")
summary(model)
correlation <- cor.test(sightseeing$平均回数, sightseeing$歴史.文化)
correlation

# 平均回数と、温泉.文化の散布図を描く
sightseeing$温泉.健康
plot(sightseeing$平均回数, sightseeing$温泉.健康, xlab = "平均回数", ylab = "温泉・健康", main = "都道府県ごとの平均回数と温泉健康")
text(sightseeing$平均回数, sightseeing$温泉.健康, labels = sightseeing$都道府県, pos = 3, cex = 0.7)
model <- lm(温泉.健康 ~ 平均回数, data = sightseeing)
abline(model, col = "red")
summary(model)
correlation <- cor.test(sightseeing$平均回数, sightseeing$温泉.健康)
correlation